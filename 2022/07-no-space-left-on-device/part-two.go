...
func main() {
  ...
  threshold := needCapacity - (diskCapacity - root.size)
  fmt.Println(findResult(root, threshold, root.size))
...
func findResult(node Node, threshold int, result int) int {
  for _, item := range node.children {
    if item.isDir {
      result = findResult(*item, threshold, result)
    }
  }

  if (node.size > threshold && node.size < result) {
    result = node.size
  }
