...
func main() {
    ...
    score := calculateScore(findBingo(numbers, boards))
    fmt.Println(score)
...
func findBingo(numbers []int, boards [][][]int) ([]int, [][]int) {
    var (
        drawnNumbers []int
        winBoard [][]int
    )
    ...
    for i := 5; i <= len(numbers); i++ {
        ...
        for i := 0; i < len(boards); i++ {
            ...
            for j := 0; j < 5; j++ {
                for k := 0; k < 5; k++ {
                    _, found := Find(drawnNumbers, boards[i][j][k])
            ...
            for j := 0; j < 5; j++ {
                for k := 0; k < 5; k++ {
                    _, found := Find(drawnNumbers, boards[i][k][j])
