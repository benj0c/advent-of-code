...
func generateLinePoints(line Line) []Point {
    ...
    if (line.start.x == line.end.x) {
        ...
        points = generateDiagonalLinePoints(line)
    }
...
func generateDiagonalLinePoints(line Line) []Point {
    var points []Point
    var absX = int(math.Abs(float64(line.start.x) - float64(line.end.x)))
    ...
    for i := 0; i <= absX; i++ {
        points = append(points, Point { line.start.x + i, line.start.y + (i*ydir) })
    }
